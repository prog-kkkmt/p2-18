...
canvas.pack()
tk.update()
# Ball, который будет отвечать за шарик 
class Ball:
    ...
    def hit_paddle(self, pos):
        ...
        if pos[2] >= paddle_pos[0] and pos[0] <= paddle_pos[2]:
            if pos[3] >= paddle_pos[1] and pos[3] <= paddle_pos[3]:
                ...
                return True
        return False
    # функция, который отвечает за движение шарика
    def draw(self):
...
class Paddle:
    ...
    def __init__(self, canvas, color):
        self.canvas = canvas
        # создаём прямоугольную платформу 10 на 100.
        self.id = canvas.create_rectangle(0, 0, 100, 10, fill=color)
        # задаём список возможных стартовых положений платформы и перемешиваем их
        ...
        self.x = 0
        self.canvas_width = self.canvas.winfo_width()
        # Стрелка вправо метод turn_right()
        self.canvas.bind_all('<KeyPress-Right>', self.turn_right)
        # Стрелка влево — turn_left()
    ...
    def start_game(self, event):
        self.started = True
    # функция, которая отвечает за движение платформы
    def draw(self):
        ...
        # если упёрлись в правую границу 
            ...
            # останавливаемся
            self.x = 0
# класс Score, который отвечает за отображение счетов
class Score:
    def __init__(self, canvas, color):
