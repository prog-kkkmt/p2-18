...
sys.implementation:
Атрибут sys.implementation представляет собой объект, содержащий информацию о реализации текущего запущенного интерпретатора Python.
...
Хотя реализация Python может использовать другое значение, если это уместно. Если для cache_tag установлено значение None, это означает, что кэширование модуля должно быть отключено.

Атрибут sys.implementation может содержать дополнительные атрибуты, специфичные для реализации Python.
Эти нестандартные атрибуты должны начинаться с подчеркивания и здесь не описаны. Независимо от содержимого sys.implementation не
изменится ни во время выполнения интерпретатора, ни между версиями реализации. Однако, может измениться между версиями языка Python.
...
'''
Значение sys.hexversion это шестнадцатеричный номер версии и представляет собой результат передачи его встроенной функции hex().
...
Обычно это первые три символа, возвращаемые sys.version. Значение sys.winver представлен в модуле sys в ознакомительных целях,
изменение этого значения не влияет на ключи реестра, используемые Python.
'''
...
Если не работает в виртуальной среде, значения останутся прежними. Если site.py обнаружит, что используется виртуальная среда,
то значения sys.prefix и sys.exec_prefix будут изменены на значения установки виртуальной среды, тогда как sys.base_prefix и
...
В частности, все файлы конфигурации (например, заголовочный файл pyconfig.h) установлены в каталоге exec_prefix/lib/pythonX.Y/config,
а модули разделяемой библиотеки установлены в exec_prefix/lib/pythonX.Y/lib-dynload, где XY номер версии Python, например 3.7.

Примечание. Если запущена виртуальная среда, то значение sys.exec_prefix будет изменено в site.py для указания на виртуальную среду.
...
что используется виртуальная среда, то значения sys.prefix и sys.exec_prefix будут изменены на значения установки виртуальной среды,
тогда как sys.base_prefix и sys.base_exec_prefix останутся указывать на базовую установку Python (ту, из которой была создана
виртуальная среда).


sys.executable:
Атрибут sys.executable это строка, задающая абсолютный путь исполняемого двоичного файла для интерпретатора Python в системах,
где это имеет смысл. Если Python не может получить реальный путь к своему исполняемому файлу, sys.executable будет пустой строкой
или None.
...
На большинстве платформ он равен lib. В Fedora и SuSE на 64-битных платформах он равен lib64, что дает следующие пути sys.path
...
# Большинство систем требуют, чтобы arg находился в диапазоне 0 - 127, и в противном случае дают неопределенные результаты.
# Некоторые системы имеют соглашение о назначении определенных значений определенным кодам выхода, но они, как правило,
# недостаточно развиты. Программы Unix обычно используют 2 для ошибок синтаксиса командной строки и 1 для всех других видов ошибок.
