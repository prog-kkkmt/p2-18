import sys  #этот модуль отвечает за определение среды запуска приложения
from Design import * #импортируем наш разработанный дизайн

class MyWin(QtWidgets.QMainWindow):                         #создаем класс приложения,который наследуется от qMainWindow
    def __init__(self):                                     #создаем конструктор для экземпляра
        QtWidgets.QWidget.__init__(self)                    #импортируем все методы библиотеки pyQt
        self.ui = Ui_MainWindow()                           #связываем дизайн с кнопками
        self.ui.setupUi(self)                               #функция отрисовки главного окна
        self.ui.pushButton_3.clicked.connect(self.setZero)  #назначаем функционал для кнопок...
    ...
    def setZero(self):                          #определяем функции...
        a = self.ui.lineEdit.text()             #создаем локальную переменную и сохраняем в нее строку из "lineEdit"
        self.ui.lineEdit.setText(a + '0')       #к "a" прибавляем '0' и сохраняем в "lineEdit"
    def setOne(self):
    ...
    def setEnter(self):
        a = self.ui.lineEdit.text()
        b = str(eval(a))            #функция "eval" преобразует строку в математическую операцию
        self.ui.lineEdit.setText(b)



if __name__ == '__main__':
    app = QtWidgets.QApplication(sys.argv) #определение в среде виндовс
    myapp = MyWin()                        #создание экземпляра окна
    myapp.show()                           #отрисовка окна на экране
    sys.exit(app.exec_())                  #устанавливает параметр завершения работы окна
